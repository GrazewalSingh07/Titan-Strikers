...
const Navbar = () => {
  ...
  return (
    <>
    <Container  maxW="container.2xl" bg="green.100" color="green.800"  fontSize="20px" margin="auto" >
    <HStack width="90%" spacing="auto">
      ...
      <Container width="container.sm" margin="auto">
          <HStack width="100%" spacing="auto">
            ...
            <Link to="/purchase-courses" element={<Mycourse/>}>My courses</Link>
            <Link to="/cart" >Cart</Link>
            
            {!isAuth?<Link to="/signup">Sign up</Link>:null } 
          {isAuth?<Button onClick={()=>dispatch(Logout(dispatch))}>Log out</Button>:<Link to="/signin">Login</Link> } 
